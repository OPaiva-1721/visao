...
**Implementado com ângulo exato, não a fórmula linear acima** (`perception/geometry.py`): a inclinação da câmera desloca a altura estimada o bastante para confundir a faixa RN-09 (~20–30 cm de erro contra faixas de ~45–75 cm). Com inclinação 0 as duas fórmulas coincidem exatamente; a derivação completa está no docstring do módulo.
...
| 1 | `core/decide.py` + `state.py` ✅ · `capture/` ✅ (`LatestFrameSlot`, `FakeFrameSource`) · `CameraFrameSource` ✅ (webcam/DroidCam via OpenCV, testado de verdade com a DroidCam do PC — 21 fps) · spike S1 ✅ · `perception/detector.py` ✅ (produz `Detection`, ainda sem distância — geometria é Fase 2) · `supervisor/` ✅ (RN-04, RN-27, RN-34 — bateria fica para a Fase 5) · `controls/` ✅ (teclado, RN-20 a RN-22) · `telemetry/` ✅ (RN-28) · `audio/engine.py` ✅ (fila P0–P4, RN-06/RN-15/RN-17, duas threads — bipe e voz — que o SO mixa; validado sem exceção/deadlock, qualidade do som ainda por ouvir com calma) · `tools/record_session.py` ✅ (grava vídeo, RN-29 — testado, 29 fps na DroidCam) · `tools/calibrate_camera.py` ✅ (mede f_px/cx_px/cy_px com o próprio Detector, sem clicar pixel — imprime pra colar no perfil) · `tools/label_events.py` ✅ (rotula intervalos por zona/faixa/lado, ADR-007 — interativo, não dá pra testar sem gravação real + humano no teclado) |
| 2 | `perception/geometry.py` ✅ (pinhole por classe, ADR-005 — ângulo exato em vez da fórmula linear, corrige a inclinação da câmera; testado sem calibração real ainda) · falta: `perception/depth`, fusão, `perception/generic`, spikes S2 e S6 · **`tools/record_trace.py` e `tools/eval.py` também entram aqui** (não em Fase 1): precisam de `Perception` com distância real |

...
**Fase 1 em andamento:** núcleo de decisão ✅ (`core/decide.py`, RN-08 a RN-16 e RN-33), captura "último frame vence" ✅ (`capture/`, ADR-004), spike S1 ✅ (**115,8 fps**, [detalhe](testes-campo/2026-09-23-spike-s1.md)), detector ✅ (`perception/detector.py`, YOLO26n + ByteTrack — produz `Detection`, sem distância ainda: isso é Fase 2), supervisor ✅ (`supervisor/monitor.py`, RN-04/RN-27/RN-34). 101 testes. Falta o motor de áudio (precisa de thread própria), a captura de câmera real, os controles, a telemetria e as ferramentas de gravação/avaliação — ver a tabela da arquitetura, seção 14.

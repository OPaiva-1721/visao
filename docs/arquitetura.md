...
| 1 | `core/decide.py` + `state.py` ✅ · `capture/` ✅ · spike S1 ✅ · `perception/detector.py` ✅ (produz `Detection`, ainda sem distância — geometria é Fase 2) · `supervisor/` ✅ (RN-04, RN-27, RN-34 — bateria fica para a Fase 5) — falta: fontes reais de câmera, `AudioEngine` (fila P0–P4, precisa de thread própria — seção 8), `controls/` (teclado), `telemetry/`, `calibrate_camera.py`, `record_session.py`, `label_events.py`, `record_trace.py`, `eval.py` |

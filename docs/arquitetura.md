...
| 2 | `perception/depth`, `perception/geometry` com fusão, `perception/generic`, spikes S2 e S6 · **`tools/record_trace.py` e `tools/eval.py` também entram aqui** (não em Fase 1): precisam de `Perception` com distância real (`d = f·H/h`), que só existe depois da geometria |
